...
fn activity_and_session_queries_cover_empty_and_cursor_edges() -> Result<(), crate::Error> {
    ...
    let before_id = segments[0]["id"].as_i64().unwrap();
    let cursor_page = rows(query(
        &test_db.db,
        "sessionsLocationSegments",
        json!({
            "beforeId": before_id,
    ...
    assert!(cursor_page
        .iter()
        .all(|row| row["id"].as_i64().unwrap() < before_id));

    let range_segments = rows(query(
        &test_db.db,
        "sessionsLocationSegmentsByDateRange",
        json!({
            "afterDate": "2026-05-14T09:00:00Z",
            "beforeDate": "2026-05-14T10:00:00Z",
            "limit": 10
        }),
    )?);
    assert_eq!(range_segments.len(), 2);

    let empty_events = rows(query(
        &test_db.db,
        "sessionsEventsForSegments",
        json!({
            "locationTags": [],
            "afterDate": "2026-05-14T08:00:00Z",
            "beforeDate": "2026-05-14T09:00:00Z"
        }),
    )?);
    ...
    let events = rows(query(
        ...
        "sessionsEventsForSegments",
        json!({
            "locationTags": ["wrld_alpha:inst-a~group(grp_alpha)"],
            "afterDate": "2026-05-14T08:00:00Z",
            "beforeDate": "2026-05-14T08:10:00Z"
        }),
    )?);
    assert!(row_texts(&events, "type").contains(&"VideoPlay".to_string()));

    let anchored = rows(query(
        &test_db.db,
        "sessionsLocationSegmentsByAnchor",
        json!({
            "sinceDate": "2026-05-14T09:00:00Z",
            "limit": 1
        }),
    )?);
