...
- **アカウント別のローカル履歴** — ゲームログなどの履歴はログイン中のアカウントごとに保存されるため、複数のアカウントを使い分けても新しい履歴が混ざりません。アップグレード後も、これまでの履歴は引き続き確認できます
- **バックアップと復元** — ワンクリックでデータを 1 つの圧縮バックアップファイルにまとめられます。自動バックアップを設定すれば、複数世代を自動で保存できます。必要なときはいつでもバックアップから復元できます
- **ワールドコレクションの共有** — お気に入りのワールドを、共有できるコレクションページにまとめられます。リンクを受け取った人はブラウザで内容を確認し、VRChat 公式サイトで各ワールドを開いたり、コレクションを VRCX-0 にインポートしたりできます。ワールドやアバター単体の VRCX-0 共有リンクも作成できます
...
後からデータの保存先を変更する場合も、新しい場所を選ぶだけでデータが自動的に移行されます。
